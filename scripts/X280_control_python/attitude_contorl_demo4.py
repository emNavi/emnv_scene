...
class pub:
    def __init__(self, uav_type, control_type):
        ...
        # subscribers
        self.local_pose_sub = rospy.Subscriber(uav_type + "_4/mavros/local_position/pose", PoseStamped, self.local_pose_callback)
        self.local_vel_sub = rospy.Subscriber(uav_type + "_4/mavros/local_position/velocity_body",TwistStamped, self.local_vel_callback)
        self.local_acc_sub = rospy.Subscriber(uav_type + "_4/mavros/super_attitude", quadrotor_msgs.msg.PositionCommand, self.local_attitude_callback)
        self.odom_sub=rospy.Subscriber(uav_type + "_4/mavros/local_position/odom",Odometry,self.odom_callback)
        self.mavros_sub = rospy.Subscriber(uav_type + "_4/mavros/state", State, self.mavros_state_callback)

        # pub
        self.target_motion_pub = rospy.Publisher(uav_type + "_4/mavros/setpoint_raw/local", PositionTarget, queue_size=100)
        self.body_target_pub = rospy.Publisher(uav_type + "_4/mavros/setpoint_raw/attitude", AttitudeTarget, queue_size=200)
        self.flightModeService = rospy.ServiceProxy(uav_type + "_4/mavros/set_mode", SetMode)
        self.armService = rospy.ServiceProxy(uav_type + "_4/mavros/cmd/arming", CommandBool)

        self.set_param_srv = rospy.ServiceProxy(uav_type+"_4/mavros/param/set", ParamSet)
